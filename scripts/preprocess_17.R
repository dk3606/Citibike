...
stations1 <- data.frame(station = citi17$startstationname, 
                        date = citi17$start_date) %>% 
  distinct()
stations2 <- data.frame(station = citi17$endstationname,
                        date = citi17$end_date) %>% 
  distinct()
...
# Weather info from NOAA in 2017 (https://w2.weather.gov/climate/xmacis.php?wfo=okx)
...
# Identify holidays (https://www.opm.gov/policy-data-oversight/pay-leave/federal-holidays/#url=2017)
Holidays17 <- date(c("2017-01-01","2017-01-02", "2017-01-16","2017-02-20","2017-05-29",
                     "2017-07-04","2017-09-04", "2019-10-09","2017-11-10","2017-11-23",
                     "2017-11-24","2017-12-25"))

### Create dataset aggregated to day
##Summary by day 
stations_day <- stations[,-3] #stations takes out the hours
stations_day <- unique(stations_day) # put it back to days (gets rid of repeat rows)

# counts the number of bikes left the station
...
# counts the number of bikes returned the station
...
# 
stations_day <- stations_day %>% filter(!(station %in% stations_closed)) #remove closed stations

stations_day <- left_join(stations_day, station_start_day, by = c("station","date")) #bikes left
stations_day <- left_join(stations_day, station_end_day, by = c("station","date")) #biked returned
stations_day <- left_join(stations_day, stops, by = "station") # capacity & radius count 

stations_day$start_day <- ifelse(is.na(stations_day$start_day), 0, stations_day$start_day) #0 NA
stations_day$end_day <- ifelse(is.na(stations_day$end_day),0, stations_day$end_day) #0 NA
stations_day$percent <- (stations_day$end_day - stations_day$start_day)/stations_day$capacity # returned - left / capacity 
...
write_csv(full_data, "data/citibike_2017.csv")
